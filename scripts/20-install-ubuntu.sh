...
if ! is_termux; then
  log "Ubuntu rootfs 安装阶段只能在 Termux 外层运行。当前运行环境：$(detect_openhouse_runtime)"
  exit 2
fi

TERMUX_HOME="${HOME:-/data/data/com.termux/files/home}"
UBUNTU_PROBE_STAGING="$TERMUX_HOME/.openhouse-bootstrap/openhouse-env-probe-ubuntu.sh"
...
EOF
  ...
  run_logged proot-distro login ubuntu -- bash -lc 'mkdir -p "$HOME/bin"; cp "/data/data/com.termux/files/home/.openhouse-bootstrap/openhouse-env-probe-ubuntu.sh" "$HOME/bin/openhouse-env-probe"; chmod 755 "$HOME/bin/openhouse-env-probe"; "$HOME/bin/openhouse-env-probe"'
